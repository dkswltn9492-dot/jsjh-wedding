...
(function () {
  ...
  function loadImagesFromFolder(folder, maxAttempts = 50) {
    return new Promise(resolve => {
        ...
        function tryNext() {
            ...
            const img = new Image();
            // 💡 1차 시도: 무조건 소문자 .jpg로 경로를 먼저 만듭니다.
            let path = `images/${folder}/${current}.jpg`;
            ...
            img.onload = function() {
             ...
             img.onerror = function() {
                // 💡 [대소문자 버그 박멸 핵심] 소문자 .jpg 실패 시, 대문자 .JPG 주소로 딱 한 번만 더 찔러봅니다!
                if (path.endsWith('.jpg')) {
                    path = `images/${folder}/${current}.JPG`;
                    img.src = path; // 대문자 주소로 재시도 유도
                    return; // 함수가 아래로 내려가서 실패 카운트 올리는 걸 막고 대기합니다.
                }
                
                // 소문자, 대문자 둘 다 뒤져도 진짜 파일이 없는 경우에만 실패로 처리합니다.
                consecutiveFails++;
                current++;
                tryNext();
            };
  ...
  function initFallingHearts() {
    ...
    const COLORS = ['#E288A9', '#FFFDF6', '#5C5457', '#9E5774', '#F2A7C3', '#221E20'];
    ...
    class Heart {
      ...
      draw() {
        ...
        const s = this.size;
        ctx.beginPath();
        ctx.moveTo(0, s * 0.1); // 윗부분 골짜기를 자연스러운 깊이로 조절
        
        // 왼쪽 곡선: 찌그러지지 않게 아래쪽(s*1.2)을 묵직하게 늘려 빵빵한 부피감을 줍니다
        ctx.bezierCurveTo(-s * 0.9, -s * 0.6, -s * 1.3, s * 0.4, 0, s * 1.2);
        
        // 오른쪽 곡선: 똑같이 풍선처럼 가득 찬 부피감으로 매칭
        ctx.bezierCurveTo(s * 1.3, s * 0.4, s * 0.9, -s * 0.6, 0, s * 0.3);
        ctx.fill();
  ...
  /* ═══════════════════════════════════════════
    ...
    // 1. 제이쿼리($) 에러 방지를 위해 querySelector로 교체
    const grid = document.querySelector('#calendarGrid');
    if (!grid) return; // 엘리먼트가 없으면 실행 중단 (안전장치)
    ...
    for (let d = 1; d <= lastDate; d++) {
      ...
      if (d === weddingDay) dayEl.classList.add('is-today'); // 결혼식 당일 표시
      dayEl.textContent = d;
    ...
    // 2. [문법 에러 박멸] 누락되었던 ${ } 템플릿 리터럴 문법 완벽 수정
    // (이 부분이 정상화되어야 하단의 지도와 갤러리 스크립트까지 안 멈추고 쭉 실행됩니다.)
    const startDate = dt.toISOString().replace(/[-:]/g, '').split('.')[0] + 'Z';
    const endDt = new Date(dt.getTime() + 2 * 60 * 60 * 1000);
    const endDate = endDt.toISOString().replace(/[-:]/g, '').split('.')[0] + 'Z';
    
    // 💡 주소 조립 문법에 문장 부호 ${ } 처리를 완벽하게 하여 자바스크립트 폭탄을 제거했습니다.
    const gcalUrl = `https://google.com{encodeURIComponent(CONFIG.groom.name + ' ♥ ' + CONFIG.bride.name + ' 결혼식')}&dates=${startDate}/${endDate}&location=${encodeURIComponent(CONFIG.wedding.venue + ' ' + CONFIG.wedding.address)}&details=${encodeURIComponent('결혼식에 초대합니다.')}`;
    
    const googleBtn = document.querySelector('#googleCalBtn');
    if (googleBtn) googleBtn.href = gcalUrl;

    // 3. ICS 다운로드 (Apple 캘린더) 연동
    const icsBtn = document.querySelector('#icsDownloadBtn');
    if (icsBtn) {
  ...
  function initLocation() {
    ...
    // 💡 혹시 모를 오타나 누락 에러를 사전에 완벽히 차단하는 안전 장치
...
bgmBtn.addEventListener('click', function() {
  ...
  async function init() {
    ...
    // 💡 [수술 완료] 자바스크립트 전체를 마비시키던 Promise.all 꼬임 구조를 완벽하게 삭제했습니다!
    // 폴더에서 사진 목록을 순수하게 1차원 배열로 아주 깨끗하게 받아옵니다.
    const galleryImages = await loadImagesFromFolder('gallery');

    // 정상적인 사진 데이터를 갤러리에 전달하여 안전하게 화면에 그립니다.
